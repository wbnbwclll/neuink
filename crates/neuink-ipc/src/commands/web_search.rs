...
use rs_trafilatura::{extract_with_options, Options as PageExtractOptions};
...
pub fn save_web_search_settings<R: Runtime>(
    ...
    if ws.enabled && ws.providers.is_empty() {
        return Err("web search cannot be enabled until at least one search provider is selected."
            .to_string());
    }
...
pub async fn test_web_search_provider<R: Runtime>(
    ...
    let count = match provider {
        SearchProvider::Duckduckgo => {
            ddgs.text_with_options(query, TextOptions::default().max_results(3)).await
...
async fn web_search<R: Runtime>(
    ...
    for provider in &providers {
        ...
        let per_provider = match provider {
            ...
            SearchProvider::Arxiv => search_arxiv(http, &query, top_k).await?,
            SearchProvider::Tavily => {
                let key = tavily_key
                    .clone()
...
async fn search_arxiv(
    ...
    let mut results = Vec::new();
    for entry in doc.descendants().filter(|node| node.is_element() && node.tag_name().name() == "entry") {
...
fn url_query_encoder(query: &str) -> String {
    ...
    reqwest::Url::parse(&format!("https://example.test/?q={}", query))
        .map(|url| url.query().unwrap_or_default().trim_start_matches("q=").to_string())
        .unwrap_or_default()
}
...
async fn read_web_page(request: ReadWebPageRequest) -> Result<ReadWebPageResponse, String> {
    ...
    let title = extracted.metadata.title.unwrap_or_default().trim().to_string();

    let char_count = text.chars().count();
    let final_chars = request
        .max_chars
...
mod tests {
    ...
    fn dedupes_by_url_keeping_provider_priority() {
        let candidates = vec![
            WebSearchResult { title: "A".into(), url: "https://x".into(), snippet: "1".into() },
            WebSearchResult { title: "B".into(), url: "https://x".into(), snippet: "2".into() },
            WebSearchResult { title: "".into(), url: "https://y".into(), snippet: "3".into() },
        ];
        let mut seen = HashSet::new();
